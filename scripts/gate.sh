...
# The current measured total is 585 (debug profile, the same `cargo test --locked --workspace`
# this arm runs): the redacted-Debug pins, provider-shape refusal checks, default OpenCode
# custody suites, and the manifest-lock contract tests compiled through both ck-auth and
# cli_opencode, and tenant plugin OAuth migration coverage.
...
run_expect 585 "workspace unit + integration" \
